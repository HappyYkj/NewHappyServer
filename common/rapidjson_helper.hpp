...
namespace rapidjson
{
    namespace detail
    {
        ...
        template<>
        inline string_view_t get_value<string_view_t>(rapidjson::Value* value, const string_view_t& dv)
        {
            if (value->IsString())
            {
                return string_view_t(value->GetString(), value->GetStringLength());
            }
            return dv;
        }
    ...
    template<typename T>
    inline T get_value(rapidjson::Value* value, string_view_t path, const T& dv = T())
    {
        auto vecs = split<string_view_t>(path, ".");
        for (auto& v : vecs)
        {
